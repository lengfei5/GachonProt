...
# f24_llr=function(x, nfreq=1)
# {
# 	n=length(x)
# 	mu=mean(x)
# 	sig2=var(x)
# 	L0 = -n/2 - n/2*log(2*pi*sig2)
# 
# 	c=cos(2*pi*nfreq/n*(0:(n-1)))
# 	s=sin(2*pi*nfreq/n*(0:(n-1)))
# 	a=mean(x*c)
# 	b=mean(x*s)
# 	x.hat=mu+2*a*c+2*b*s
# 	sig2.1=var(x-x.hat)
# 
# 	L1 = -n/2 - n/2*log(2*pi*sig2.1)
# 	D=2*(L1-L0)	
# 	c(D, p=pchisq(D, 2, lower.tail = FALSE, log.p = FALSE))
# }
...
#f24_R2_new = function(x, t=2*(0:(length(x)-1)), period=24, offset=0)
#{
#	n=length(x)
#	sig2=var(x)
#	nn = 2048
#	fit0 = nls(x ~ mu,start = list(mu = 1),control= (warnOnly = T))
#	fit1=nls(x ~ mu+A*cos(2*pi/period*(t-phi)),start = list(mu = 1, A=0.01, phi=0), control=(warnOnly = T))
#	ss = lrtest(fit0,fit1)
#	pvalue = ss$'Pr(>Chisq)'[2]
#	c(Pvalue_alt = pvalue)
	
#	R2=0
#	if(sig2>0) R2 =1.0-sum(residuals(fit)^2)/(n-1)/sig2
#	p=3
#	pv=pbeta(R2, (p-1)/2, (n-p)/2, lower.tail = FALSE, log.p = FALSE)
#	
#	lu=length(unique(x))
...
#	phase=(phase+offset)%%period
#x=2^x
#	c(lu=lu, mean=mean(x), amp=2*sqrt(a^2+b^2), relamp=(max(x)-min(x))/(2.0*fit$coef[1]), phase=phase, pval=pv)
...
first_upper = function(test)
{
	ttest = tolower(test)
	ttest = c(toupper(unlist(strsplit(as.character(ttest),""))[1]), unlist(strsplit(as.character(ttest),""))[-1])
	ttest = paste(ttest, collapse = '')
	return(ttest)
}
...
# for(n in 1:5000)
# {
# 	if(n<4000) x=1+0*cos(2*pi/N*c(0:(N-1)))+rnorm(N)*0.5
# 	else x=1+0.5*cos(2*pi/N*c(0:(N-1)))+rnorm(N)*0.5
# 
# 	p1=f24_R2(x, t=2*(0:11))[6]
# 	p2=f24(x)[6]
# 	pvals=rbind(pvals, c(ll=p1,fish=p2))
# }
# 
# f=c(rep(0,4000), rep(1,1000))
# boxplot(as.vector(-log(pvals)) ~ as.factor(c(f,f+2)))
